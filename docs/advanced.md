...
| [`app/extensions/llm_adapter.py`](../app/extensions/llm_adapter.py) | Gemini / AiHubMix / GLM / DeepSeek / OpenAI 兼容适配 |
...
### 1. GLM / DeepSeek / OpenAI 不是简单改个 Base URL 就能用

`hcaptcha-challenger` 内部调用的是 `google-genai` 风格的多模态接口。  
所以接 GLM、DeepSeek 或 OpenAI 时，不能只把 `GEMINI_BASE_URL` 改成对应供应商地址。

真正要做的是保留上层调用方式，再在适配层里把图片、消息和结构化输出转换成对应接口能接受的格式。
...
4. GLM / DeepSeek / OpenAI / Gemini 的响应格式是否变化。
