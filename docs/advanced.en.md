...
| [`app/extensions/llm_adapter.py`](../app/extensions/llm_adapter.py) | Gemini / AiHubMix / GLM / DeepSeek / OpenAI compatibility adapter |
...
### 1. GLM / DeepSeek / OpenAI is not a simple Base URL replacement

`hcaptcha-challenger` internally depends on a `google-genai`-style multimodal interface.

That means you cannot support GLM, DeepSeek, or OpenAI by only changing `GEMINI_BASE_URL` to a provider endpoint.

The actual work is to preserve the upper-layer call pattern while converting images, messages, and structured outputs into a format the target endpoint accepts in the adapter layer.
...
4. Whether GLM / DeepSeek / OpenAI / Gemini response formats change again.
